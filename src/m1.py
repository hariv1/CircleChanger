...
class CircleChanger(object):
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        random_thickness = random.randrange(3, 16)
        self.circle.outline_thickness = random_thickness

        random_index = random.randrange(0, len(self.colors))
        self.circle.fill_color = self.colors[random_index]
    ...
    def swallow(self, other_circle_changer):
        ...
        new_center = self.circle.center.halfway_to(
            other_circle_changer.circle.center)

        new_radius = (self.circle.center.get_distance_from(
            other_circle_changer.circle.center))/2

        return CircleChanger(new_center.x, new_center.y, new_radius,
